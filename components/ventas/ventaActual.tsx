...
const VentaActual = ({
  ...
  const total = Number(
    productosEnVenta.reduce(
      (sum, item) => sum + Number(item.price) * item.quantityInSale,
      0
    ).toFixed(2)
  );
    ...
    <>
      <View style={styles.card}>
        ...
        {productosEnVenta.length === 0 ? (
          ...
          <View testID="venta-actual-container">
            {productosEnVenta.map((item) => (
              <View key={item.id} style={styles.saleItem} testID={`sale-item-${item.id}`}>
                <View style={styles.saleItemInfo}>
                  <ThemedText 
                    style={styles.saleItemName}
                ...
                <View style={styles.quantityControls}>
                  <TouchableOpacity
                    ...
                    testID={`decrease-button-${item.id}`}
                  >
                  ...
                  <TouchableOpacity
                    onPress={() => handleQuantityChange(item.id, 1)}
                    style={styles.quantityButton}
                    testID={`increase-button-${item.id}`}
                  >
                    <ThemedText style={styles.quantityButtonText}>+</ThemedText>
                  </TouchableOpacity>
                </View>
                <ThemedText style={styles.saleItemTotal} testID={`sale-item-total-${item.id}`}>
            ...
            <View style={styles.totalContainer}>
              ...
              <ThemedText style={styles.totalTextValue} testID="total-value">
                ${total}
              </ThemedText>
            </View>
            <View style={styles.actionButtonsContainer}>
              <TouchableOpacity
                ...
                onPress={handleVentaNormal}
                testID="venta-normal-button"
              >
