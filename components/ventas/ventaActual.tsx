...
const VentaActual = ({
 ...
 handleVender: (productos: VentaProduct[], total: number, owner_id: string) => Promise<void>;
}) => {
 const {user} = useAuth()


 const total = Number(productosEnVenta.reduce(
   (sum, item) => sum + Number(item.price) * item.quantityInSale, 0
 ).toFixed(2));
 const handleVentaNormal = () => {
   ...
   handleVender(productosEnVenta, total, user.id);
 };
         ...
         <View>
           {productosEnVenta.map((item) => (
             <View key={item.id} style={styles.saleItem}>
               ...
               <ThemedText style={styles.saleItemTotal} testID="sale-item-total">
